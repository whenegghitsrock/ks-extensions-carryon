...
扩展组件安装时，驱动的镜像版本与系统及内核版本有关，如当前系统为 `Ubuntu 22.04`， 内核版本为 `5.15.0-131-generic`，则对应镜像的 tag 为 `5.15.0-131-generic-ubuntu22.04`， 驱动镜像为 `kubesphereon/driver:5.15.0-131-generic-ubuntu22.04`，请将该驱动推送至本地私有镜像仓库。

并且，将以下镜像推送至您的本地私有镜像仓库。

```txt
kubesphereon/node-feature-discovery:v0.14.2
kubesphereon/dcgm:3.3.0-1-ubuntu22.04
kubesphereon/gpu-operator-validator:v23.9.2
kubesphereon/k8s-cc-manager:v0.1.1
kubesphereon/k8s-driver-manager:v0.6.5
kubesphereon/k8s-kata-manager:v0.1.2
kubesphereon/k8s-mig-manager:v0.6.0-ubuntu20.04
kubesphereon/vgpu-device-manager:v0.2.4
kubesphereon/cuda:12.3.2-base-ubi8
kubesphereon/gpu-feature-discovery:v0.8.2-ubi8
kubesphereon/gpu-operator:v23.9.2
kubesphereon-device-plugin:v0.14.5-ubi8
kubesphereon/container-toolkit:v1.14.6-ubuntu20.04
kubesphereon/dcgm-exporter:3.3.0-3.2.0-ubuntu22.04
kubesphereon/kubevirt-gpu-device-plugin:v1.2.4
```

最后，修改扩展组件配置，将多个默认镜像地址替换为本地镜像仓库地址。假如本地镜像仓库地址为 `registry.cn-beijing.aliyuncs.com/kse`，需将默认镜像地址，如 `kubesphereon/gpu-operator:v23.9.2` 修改为 `registry.cn-beijing.aliyuncs.com/kse/nvidia/gpu-operator:v23.9.2`。
...
spec:
  ...
  containers:
  - name: cuda-vectoradd
    image: "kubesphereon/cuda-sample:vectoradd-cuda11.7.1-ubuntu20.04"
    resources:
      limits:
        nvidia.com/gpu: 1
EOF
